...
def _new_group_via_split_group(
    ...
    # The torchcomms gloo (CPU) backend rejects calls to `split_group` from
    # ranks that aren't in the requested split, so short-circuit non-members
    # here. NCCL/XCCL split_group is collective across the parent group, so
    # we must NOT skip the call when an accelerator is available.
    if not torch.accelerator.is_available() and default_pg.rank() not in group_ranks:
