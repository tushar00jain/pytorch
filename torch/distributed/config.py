...
# When enabled, pipeline stages carry forward (downstream) and backward
# (upstream) P2P on two separate communicators instead of sharing one. A single
# PP communicator serializes all send/recv in one FIFO: coalescing makes a single
# mixed batch deadlock-free, but across batches (pipeline skew, looped / V
# schedules, skip connections) the shared FIFO can still form a dependency cycle
# and deadlock. Splitting by direction removes that hazard and restores
# full-duplex bandwidth. PipelineStage(p2p_per_direction=...) overrides this
# default per stage. Requires a device-bound default process group.
