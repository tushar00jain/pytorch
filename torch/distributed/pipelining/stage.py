...
class _PipelineStageBase(ABC):
    ...
    def __init__(
        ...
        p2p_per_direction: bool | None = None,
    ):
        """
        Args:
            ...
            p2p_per_direction: If ``True``, carry forward (downstream) and
                ...
                group (see :func:`_build_p2p_direction_groups`). When ``None``
                (the default) the value of
                ``torch.distributed.config.pipeline_per_direction_p2p`` is used,
                so it can be enabled globally without threading a flag through
                framework code.
        """
        super().__init__()
        ...
        # byte-for-byte unchanged. ``None`` defers to the global config flag so
        # the feature can be turned on without framework changes.
        if p2p_per_direction is None:
            import torch.distributed.config as dist_config

            p2p_per_direction = dist_config.pipeline_per_direction_p2p
