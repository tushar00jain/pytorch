...
def _build_p2p_direction_groups(
    ...
    assert fwd is not None and bwd is not None  # noqa: S101
    logger.info(
...
class _PipelineStageBase(ABC):
    ...
    def __init__(
        ...
        # Forward (downstream) and backward (upstream) P2P communicators. Enabled
        # globally via torch.distributed.config.pipeline_per_direction_p2p (env
        # TORCH_DISTRIBUTED_PIPELINE_PER_DIRECTION_P2P); when disabled both alias
        # ``self.group`` so behavior is byte-for-byte unchanged.
        import torch.distributed.config as dist_config

        self.p2p_per_direction = dist_config.pipeline_per_direction_p2p
        self._fwd_group: dist.ProcessGroup | None
        self._bwd_group: dist.ProcessGroup | None
    ...
    def _get_init_p2p_neighbors_ops(self) -> list[dist.P2POp]:
        ...
        # Separate recv buffers per direction: with per-direction P2P the forward
        # and backward recvs run concurrently on different communicators/streams,
        # so they must not share a buffer (concurrent writes = data race). The
        # send buffer is only read, so it can be shared.
        fwd_recv_tensor = torch.zeros(1, device=self.device, dtype=torch.float32)
        bwd_recv_tensor = torch.zeros(1, device=self.device, dtype=torch.float32)
