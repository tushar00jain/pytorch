...
def _config_activation_worker(rank: int, world_size: int, q):
    ...
    # Turn the feature on globally -- caller passes no p2p_per_direction arg.
...
def test_config_flag_activates_without_explicit_arg():
    """torch.distributed.config.pipeline_per_direction_p2p turns the feature on
    with no constructor arg -- so frameworks need no code change to enable it."""
    _require_n_gpus(2)
    results = _run_config_activation(world_size=2)
    assert results and all(results.values()), (
        f"config flag did not activate per-direction comms on all ranks: {results}"
    )
...
if __name__ == "__main__":
    ...
    act = _run_config_activation()
    print(f"==> config-flag activation (no arg): {act}", flush=True)
    ok = bs == "completed" and fs == "completed" and act and all(act.values())
    if ok:
        try:
            _assert_match(base, fix)
