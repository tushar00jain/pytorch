...
The fix -- per-direction communicators (upstream
``PipelineStage(p2p_per_direction=True)``; here exercised at the primitive level
by ``_run_ring_per_direction``) -- puts forward (``down``) and backward (``up``)
P2P on separate communicators/streams, so neither can block the other in a shared
FIFO. With per-direction comms, the ring completes.
...
def _run_ring_per_direction(rank: int, world_size: int) -> None:
    ...
    r->r-1 traffic. Forward and backward P2P then live on separate comms/streams,
    so neither can block the other in a FIFO -> no cross-batch cycle.

    This mirrors what PipelineStage(p2p_per_direction=True) does upstream; here we
    build the two comms with plain ``dist`` primitives so the ring test stays
    self-contained. (PipelineStage uses split_group, which composes with PP as a
    sub-axis of a larger mesh; for this pure-PP ring, new_group over all ranks is
    equivalent and group-collective.)
    """
    import torch.distributed as dist
...
def _ring_worker(
    ...
    env_on = os.environ.get(
        "TORCH_DISTRIBUTED_PIPELINE_PER_DIRECTION_P2P", "0"
    ).lower() in ("1", "true", "yes", "on")
    use_fix = per_direction or env_on
    if rank == 0:
