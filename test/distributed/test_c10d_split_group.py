...
class SplitGroupBackendWrapperTest(SplitGroupTestBase):
    ...
    def test_split_group_single_backend(self):
        ...
        dist.all_reduce(gpu_tensor, group=split_pg)
        expected = torch.ones(3, 3, device=device) * self.world_size
        self.assertEqual(gpu_tensor, expected)

        # Actual split into two halves
        split_size = self.world_size // 2
        pg_ranks_by_dim = torch.arange(self.world_size).view(2, split_size)
        split_pg_2 = dist.split_group(split_ranks=pg_ranks_by_dim.tolist())

        gpu_tensor = torch.ones(3, 3, device=device)
        dist.all_reduce(gpu_tensor, group=split_pg_2)
        expected = torch.ones(3, 3, device=device) * split_size
        self.assertEqual(gpu_tensor, expected)
