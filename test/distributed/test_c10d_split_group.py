...
try:
    ...
    _DIST_CONFIG_AVAILABLE = True
except ImportError:
    _DIST_CONFIG_AVAILABLE = False
...
class SplitGroupTestBase(MultiProcessTestCase):
    ...
    def _init_pg(self, backend, use_device_id=True):
        ...
        dist.init_process_group(
            ...
            rank=self.rank,
            store=store,
            device_id=device if use_device_id else None,
        )
        return device

    def _verify_gpu_allreduce(self, split_pg, device, expected_world_size):
        """Verify all_reduce works on GPU for a split process group."""
    ...
    def _verify_allreduce_on_split(self, split_pg, device, expected_world_size):
        """Verify all_reduce works on both GPU and CPU for a split process group."""
        self._verify_gpu_allreduce(split_pg, device, expected_world_size)

        cpu_tensor = torch.ones(10)
        dist.all_reduce(cpu_tensor, group=split_pg)
        self.assertTrue(torch.all(cpu_tensor == expected_world_size))
...
class SplitGroupLargerScaleTest(SplitGroupTestBase):
    """Tests for split_group with more GPUs to verify actual sub-group splitting."""
...
@unittest.skipIf(not _DIST_CONFIG_AVAILABLE, "dist_config not available")
class SplitGroupBackendWrapperTest(SplitGroupTestBase):
    ...
    @_with_torchcomm_env
    @skip_if_lt_x_gpu(4)
    def test_split_group_single_backend(self):
        """Test split_group with single nccl backend via BackendWrapper on default pg."""
        device = self._init_pg("cuda:nccl")

        # No-op split: all ranks in one group
        all_ranks = list(range(self.world_size))
        split_pg = dist.split_group(split_ranks=[all_ranks])
        self._verify_gpu_allreduce(split_pg, device, self.world_size)

        # Actual split into two halves
        split_pg_2 = dist.split_group(split_ranks=self._split_into_halves())
        self._verify_gpu_allreduce(split_pg_2, device, self.world_size // 2)
    ...
    @_with_torchcomm_env
    @skip_if_lt_x_gpu(4)
    def test_split_group_multi_backend(self):
        """Test split_group with cpu:gloo,cuda:nccl splits both backends via BackendWrapper on default pg."""
        device = self._init_pg("cpu:gloo,cuda:nccl", use_device_id=False)

        split_size = self.world_size // 2
        pg_ranks_by_dim = torch.arange(self.world_size).view(2, split_size)
        split_pg = dist.split_group(split_ranks=pg_ranks_by_dim.tolist())
    ...
    @_with_torchcomm_env
    @skip_if_lt_x_gpu(4)
    def test_split_group_multi_backend_no_op(self):
        """Test no-op split (all ranks) with multi-backend via BackendWrapper on default pg."""
        device = self._init_pg("cpu:gloo,cuda:nccl", use_device_id=False)
