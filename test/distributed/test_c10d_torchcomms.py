...
class TestC10dTorchCommsSplitMixedBackends(C10dTorchCommsTestBase):
    """Verify split works when the parent PG mixes torchcomms and non-torchcomms backends.

    The parent PG uses ``cuda:nccl`` (a torchcomms ``BackendWrapper``) with
    ``cpu:fake`` (a plain ``FakeProcessGroup``).  The backend auto-qualification
    in ``_new_group_via_split_group`` narrows the split to only the default
    device backend, so the fake CPU backend is never asked to split.
    """
